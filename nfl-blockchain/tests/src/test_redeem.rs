...
fn test_redeem_yes() {
    ...
    // Create user ATAs for collateral, YES, and NO
    ...
    // Mint collateral to user (100 USDC)
    ...
    // Assert balances
    let vault_acc: TokenAccount = program.account(vault.pubkey()).unwrap();
    ...
    assert_eq!(vault_acc.amount, 0); // all collateral redeemed
    assert_eq!(user_collateral.amount, 100); // original amount restored
    assert_eq!(user_yes.amount, 0); // all burned in redeem
    assert_eq!(user_no.amount, 10); // loser tokens unaffected
